...
public class DatabaseInitializer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        jdbcTemplate.execute("UPDATE jugadores SET precio_base = 35000 WHERE (nickname LIKE 'Caps%' OR nickname LIKE 'BrokenBlade%' OR nickname LIKE 'Razork%' OR nickname LIKE 'Humanoid%' OR nickname LIKE 'Photon%') AND precio_base < 35000");
        jdbcTemplate.execute("UPDATE jugadores SET precio_base = 15000 WHERE precio_base = 5000 AND nickname NOT LIKE 'Caps%' AND (nickname LIKE 'Elyoya%' OR nickname LIKE 'Supak%' OR nickname LIKE 'Caliste%')");
        
        jdbcTemplate.execute("UPDATE jugadores SET precio_actual = precio_base WHERE precio_actual IS NULL OR precio_actual = 0");
    ...
    private void prepararEscenarioDemo() {
        ...
        jdbcTemplate.execute("INSERT IGNORE INTO usuarios (id, email, password, nombre, nickname, created_at, updated_at) VALUES " +
        ...
        jdbcTemplate.execute("INSERT IGNORE INTO ligas (id, nombre, codigo_acceso, admin_id, created_at, updated_at) VALUES " +
                "(100, 'Liga de Evaluación TFG', 'DEMO2026', 100, NOW(), NOW())");

        // 4. Crear Equipos (Presupuesto 40k según FASE 1)
        jdbcTemplate.execute("INSERT IGNORE INTO equipos_usuarios (id, nombre_equipo, presupuesto_disponible, puntuacion_total, liga_id, usuario_id, created_at, updated_at) VALUES " +
