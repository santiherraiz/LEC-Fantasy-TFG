...
public class JornadaService {
    ...
    public void sincronizarCalendario() {
        ...
        // OverviewPage es el campo estable para 2026 en MatchSchedule
        ...
        try {
            ResponseEntity<MatchScheduleResponse> response = restTemplate.exchange(url, HttpMethod.GET,
                    new HttpEntity<>(headers), MatchScheduleResponse.class);

            if (response.getBody() == null || response.getBody().getCargoquery() == null
                    || response.getBody().getCargoquery().isEmpty()) {
    ...
    private void procesarRespuestaCargo(MatchScheduleResponse response) {
        ...
        for (MatchScheduleResponse.CargoItem item : response.getCargoquery()) {
            ...
            if (data.getDateTimeUtc() == null || data.getMatchId() == null)
                continue;
