...
public class PuntuacionService {
    ...
    @Autowired
    private EquipoLecRepository equipoLecRepository;
    @Autowired
    ...
    public int obtenerSemanaActual() {
        // Primero, intentamos obtener la semana más alta que ya tenga estadísticas
        // calculadas
    ...
    public void resetTotal() {
        ...
        historicoAlineacionRepository.findAllWithEntities().forEach(h -> {
            h.setPuntosSemanales(0.0);
            historicoAlineacionRepository.save(h);
        });
    ...
    public String calcularPuntos() {
        ...
        for (String serieId : seriesPendientes) {
            ...
            if (!terminada && mapasSerie.size() < 3)
                continue;
            ...
            for (List<EstadisticaPartido> statsJugador : porJugador.values()) {
                ...
                for (EstadisticaPartido s : statsJugador) {
                    ...
                    double ptsReales = (s.getKills() * 3.0) + (s.getAssists() * 1.5) + (s.getDeaths() * -1.0)
                            + (s.getCs() * 0.02) + (victoria ? 5.0 : 0.0);
    ...
    private double calcularPuntosJugadorEnJornada(Long jugadorId, Long jornadaId) {
        List<EstadisticaPartido> stats = estadisticaPartidoRepository.findByJugadorId(jugadorId).stream()
                .filter(s -> s.getPartido().getJornada() != null
                        && s.getPartido().getJornada().getId().equals(jornadaId))
                .collect(Collectors.toList());
        if (stats.isEmpty())
            return 0.0;

        Map<String, List<EstadisticaPartido>> porSerie = stats.stream()
                .collect(Collectors.groupingBy(s -> s.getPartido().getSerieId() != null ? s.getPartido().getSerieId()
                        : s.getPartido().getGameId()));
        double sumaSeries = 0;
    ...
    public void importarPartidosDeLeaguepedia() {
        ...
        h.set(HttpHeaders.COOKIE, MIS_COOKIES);
        try {
            ...
            if (body != null && body.getCargoquery() != null) {
                for (PartidoLeaguepediaDTO.CargoItem item : body.getCargoquery()) {
                    ...
                    if (po.isPresent() && po.get().isPuntosCalculados())
                        continue;

                    Partido p = po.orElse(new Partido());
                    p.setGameId(d.getGameId());
                    p.setTeam1(d.getTeam1());
                    p.setTeam2(d.getTeam2());
                    ...
                    if (d.getDateTimeUtc() != null) {
                        ...
                        if (fechaPartido.isBefore(clockService.ahora())) {
                            ...
                            // Si aún es "futuro", nos aseguramos de que no tenga ganador (por si el reloj
                            // se mueve atrás)
                            p.setWinTeam(null);
                            p.setLossTeam(null);
                    ...
                    p.setSerieId(
                            d.getGameId().contains("_") ? d.getGameId().substring(0, d.getGameId().lastIndexOf("_"))
                                    : d.getGameId());
    ...
    public void importarEstadisticasDeLeaguepedia() {
        ...
        for (Partido p : pendientes) {
            ...
            try {
                String url = "https://lol.fandom.com/api.php?action=cargoquery&format=json&tables=ScoreboardGames=SG,ScoreboardPlayers=SP&fields=SP.GameId,SP.Link,SP.Team,SP.Kills,SP.Deaths,SP.Assists,SP.CS,SP.Gold&join_on=SG.GameId=SP.GameId&where=SG.GameId='"
                        + p.getGameId() + "'&limit=500";
                HttpHeaders h = new HttpHeaders();
                h.set("User-Agent", "Mozilla/5.0");
                h.set(HttpHeaders.COOKIE, MIS_COOKIES);
                ResponseEntity<String> res = rt.exchange(url, HttpMethod.GET, new HttpEntity<>(h), String.class);
                    ...
                    transactionTemplate.execute(status -> {
                        for (MatchDataResponse.CargoItem i : r.getCargoquery()) {
                            ...
                            jugadorRepository.findByNickname(nick).ifPresent(jug -> {
                                if (!estadisticaPartidoRepository.existsByPartidoGameIdAndJugadorId(p.getGameId(),
                                        jug.getId())) {
